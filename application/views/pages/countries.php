...
<div class="dtHorizontalVerticalExampleWrapper">
    <?php echo form_open_multipart('', array('id' => 'add_country')); ?>
        ...
        width="100%">
            <thead>
              ...
              <tr class="insert no-sort">
                <td></td>
                <td></td>
                <td><div contenteditable spellcheck="false" class="editable" id="data1" name="country"></div></td>
                <?php
                  if($accountTypeSession != 'User'){
...
$(document).ready(function () {
  ...
  var dataTable = $('#dtHorizontalVerticalExample').DataTable({
    ...
    order: [[ 1, "asc" ]],
    ajax: {
      url: '<?php echo base_url(); ?>countries/showAllCountries',
      type: 'POST',
      data: { checker: "check" }
      },
      columnDefs: [{
    ...
    $('#add_country').on('submit', function(e){
      ...
      var country = $('#data1').text();
      var status = 'Active';

      if(country != '' && status != ''){
        $.ajax({
          url: "<?php echo base_url(); ?>countries/addCountry",
          method: "POST",
          data: {
    ...
    function update_country(id, column, value)
    {
      $.ajax({
        url: "<?php echo base_url(); ?>countries/updateCountry",
        method: "POST",
    ...
    dataTable.on('blur', '.update', function() {
      ...
      if(value != oldValue && value != ''){
        ...
        update_country(id, column, value);
      }
    });
          ...
          if(affectedItems <= 0){
            <?php
              if($accountTypeSession == 'Administrator' || $accountTypeSession == 'Super-Administrator'){
                echo "msg = 'WARNING: This operation is irreversible, once deleted country can\'t be restored again.\\n\\nContinue deleting this country?';";
              } else{
                echo "msg = 'Are you sure you want to remove this country?';";
              }
            ?>
            if(confirm(msg)){
              $.ajax({
                url:"<?php echo base_url(); ?>countries/deleteCountry",
                method:"POST",
                data:{id:id},
                success:function(data){
                  reloadTable();
      ...
      $.ajax({
        ...
        success:function(affectedItems){
          if(affectedItems <= 0){
            <?php
              if($accountTypeSession == 'Administrator' || $accountTypeSession == 'Super-Administrator'){
                echo "msg = 'WARNING: This operation is irreversible, once deleted country/s can\'t be restored again.\\n\\nContinue deleting selected country/s?';";
              } else{
                echo "msg = 'Are you sure you want to remove selected country/s?';";
              }
            ?>
            if(confirm(msg)){
              $.ajax({
    ...
    dataTable.on('click', '.restore', function () {
      if(confirm("Are you sure you want to restore this country?")){
        var id = [];
        id[0] = $(this).attr("id");

        $.ajax({
          url:"<?php echo base_url(); ?>countries/restoreCountry",
          method:"POST",
          data:{id:id},
          success:function(data){
            reloadTable();
    ...
    $('.restore-all').on('click', function(){
      if(confirm("Are you sure you want to restore selected country/s?")){
        var id = [];
